<?php	
class Mkpicompare extends CI_Model{
	function fget_first_step_region($fdate, $ldate, $bts_type){
		$peca =explode(" ", $fdate);
		$fdate2 = $peca[0];
		...
		if($bts_type == 'KPI_XL' || $bts_type == 'KPI_AXIS'){
			...
			WHERE
			((Substr(p.pperiod, 3,6)) = '$fdate2'
            AND
            (Substr(u.uperiod, 3,6)) = '$fdate2')
			OR
			((Substr(p.pperiod, 3,6)) = '$ldate2'
             AND
            (Substr(u.uperiod, 3,6)) = '$ldate2') 
